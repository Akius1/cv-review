...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log('=== Expert Meetings API Debug ===')
    ...
    // Build query - Select meeting bookings for this expert
    let query = supabase
    ...
    const processedMeetings = (meetings || []).map(meeting => ({
      ...meeting,
      applicant_name: meeting.applicant 
        ? `${meeting.applicant.first_name || ''} ${meeting.applicant.last_name || ''}`.trim() 
        : 'Unknown Applicant',
      applicant_email: meeting.applicant?.email,
    ...
    console.error('Expert Meetings GET error:', error)
    return new NextResponse(
      JSON.stringify({ 
