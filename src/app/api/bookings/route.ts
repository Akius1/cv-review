...
// Function to get Google tokens from expert users
async function getExpertGoogleTokens(): Promise<{ accessToken: string; refreshToken: string } | null> {
  try {
    // Get the first expert user with Google tokens
    ...
    if (error || !tokens) {
      console.log('No expert Google tokens found');
      return null;
    }
    ...
    if (isExpired) {
      console.log('Expert Google tokens are expired');
      return null;
    }
    ...
    console.error('Error getting expert Google tokens:', error);
    return null;
  }
}
...
async function generateMeetingLink(
  ...
  // Try to get expert Google tokens
  const expertTokens = await getExpertGoogleTokens();
  ...
  if (expertTokens) {
    try {
      ...
      const result = await createGoogleMeetEvent(
        expertTokens.accessToken,
        expertTokens.refreshToken,
        expertEmail,
        applicantEmail,
        expertName,
        applicantName,
    ...
    console.log('No expert Google tokens available, using Jitsi Meet');
  }
